...
from hive.integrations._github_repos import all_repos, clear_cache, parse_repo_ref, validate_repo
...
def test_all_repos_prefers_user_oauth_token(monkeypatch):
    ...
    monkeypatch.setattr("hive.integrations._github_repos.httpx.get", fake_get)
...
def test_all_repos_uses_gh_cli_and_caches(monkeypatch):
    ...
    monkeypatch.setattr("hive.integrations._github_repos.subprocess.run", fake_run)
    monkeypatch.setattr("hive.integrations._github_repos._gh_active_login", lambda: "")
