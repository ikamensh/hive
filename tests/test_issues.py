...
def test_resolve_issue_close_is_idempotent_when_already_closed(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr("hive._workstreams.issues.httpx.post", lambda *a, **k: Resp(201, {}))
    monkeypatch.setattr(
        "hive._workstreams.issues.httpx.patch",
        lambda *a, **k: calls.append("patch") or Resp(422, {"message": "Validation Failed"}),
    )
    monkeypatch.setattr("hive._workstreams.issues.httpx.get", lambda *a, **k: Resp(200, {"state": "closed"}))
...
def test_delete_branch_is_idempotent_when_branch_already_gone(monkeypatch):
    ...
    seen = []
    monkeypatch.setattr(
        "hive._workstreams.issues.httpx.delete",
...
def test_delete_branch_raises_on_unexpected_status(monkeypatch):
    monkeypatch.setattr(
        "hive._workstreams.issues.httpx.delete", lambda *a, **k: _DeleteResp(500, "server error")
    )
...
def test_scan_downloads_attachments_and_serves_to_runner(app, monkeypatch):
    ...
    monkeypatch.setattr("hive._workstreams.issues.httpx.get", lambda *a, **k: FakeResp())
    resp = client.post(f"/api/projects/{pid}/scan-issues").json()
...
def test_cancel_issue_run_task_cancels_parent_run(app, monkeypatch):
    """Cancelling a child resolve/review task is also a run cancellation.

    Operators used the generic task-cancel surface before issue runs had their
    own visible cancel control; the parent run must not stay active and later
    dispatch more selected issues.
    """
    client, store = app
    project = client.post("/api/projects", json={"name": "spec"}).json()
    client.patch(f"/api/projects/{project['id']}", json={"spec_repo": "https://github.com/o/r.git"})
    ...
    monkeypatch.setattr(
        "hive.api.fetch_open_issues_full",
        lambda repo, token: [issue(1, "first"), issue(2, "second"), issue(3, "third"), issue(4, "fourth")],
    )
    ...
    run = client.post(
        f"/api/projects/{project['id']}/workstreams/{stream['id']}/issue-runs",
        json={"scope": "selected", "issue_numbers": [1, 2, 3, 4]},
    ).json()["run"]
    task = store.list(Task, project_id=project["id"])[0]

    cancelled_task = client.post(f"/api/tasks/{task.id}/cancel").json()
    project_after = client.get(f"/api/projects/{project['id']}").json()
    run_after = next(r for r in project_after["issue_runs"] if r["id"] == run["id"])

    assert cancelled_task["status"] == TaskStatus.cancelled
    assert run_after["status"] == IssueRunStatus.cancelled
    assert run_after["counts"]["cancelled_tasks"] == 1
    assert run_after["counts"]["running"] == 0
    assert run_after["counts"]["queued"] == 4
...
def test_cancelled_delivered_issue_task_result_refreshes_parent_run(app):
    client, store = app
    project = client.post("/api/projects", json={"name": "spec"}).json()
    client.patch(f"/api/projects/{project['id']}", json={"spec_repo": "https://github.com/o/r.git"})
    stream = next(w for w in client.get(f"/api/projects/{project['id']}").json()["workstreams"] if w["kind"] == "github_issues")
    ...
    client.post(f"/api/issue-runs/{run.id}/cancel")
    client.post(f"/api/tasks/{task.id}/result", json={"text": "stopped", "cancelled": True}, headers=RUNNER_HEADERS)

    run_after = store.get(IssueRun, run.id)
    assert run_after.status == IssueRunStatus.cancelled
    assert run_after.counts["running"] == 0
    assert run_after.counts["queued"] == 1
...
def test_preflight_checks(monkeypatch):
    from hive._workstreams.preflight import preflight_checks
    ...
    monkeypatch.setattr("hive._workstreams.preflight.repo_permissions",
...
def test_directive_lands_and_completes(app, monkeypatch):
    ...
    _report(
        ...
        resolve["id"],
        "Added the flag.",
        structured_result={"task_id": resolve["id"], "outcome": "fixed",
                           "tests_run": ["pytest"], "branch_pushed": True},
    )
    _pump(client, store)
    review = _poll(client, rid)
    ...
    _report(
        ...
        review["id"],
        "Verified.",
        structured_result={"task_id": review["id"], "outcome": "accept",
                           "tests_run": ["pytest"], "changes_pushed": False},
    )
