...
def escalate(store, title: str, instructions: str, project_id: str = "") -> HumanTask | None:
    ...
    for task in store.list(HumanTask):
        if (
            ...
            return None
    return store.put(HumanTask(project_id=project_id, title=title, instructions=instructions))
