...
def escalate(
    ...
    *,
    ...
    is an org-wide todo; `assignee_user_id` names the one user who can act
    (e.g. the machine owner for a login fix), empty means any admin. Returns
    the task it filed, or None if a matching one was already open (re-pointing
    its assignee when ownership changed)."""
    for task in store.list(HumanTask, workspace_id=workspace_id, status=HumanTaskStatus.open):
        if (dedup_key and task.dedup_key == dedup_key) or (
            task.title == title and task.project_id == project_id
        ):
            if assignee_user_id and task.assignee_user_id != assignee_user_id:
