...
If BLOCKED: make no code changes. State the specific decision or unavailable prerequisite in `blocking_question`, with your recommended default — this report is exactly what the human sees to unblock the item. End your report with `OUTCOME: BLOCKED`.
...
If the session ends with implementation, tests, or push still unfinished, preserve the checkout and report `incomplete` with concrete `remaining_work`. This is execution progress, not a missing product decision. End with `OUTCOME: INCOMPLETE`.

End with exactly one line, nothing after it:
`OUTCOME: BLOCKED`, `OUTCOME: FIXED`, or `OUTCOME: INCOMPLETE`
