...
`hive --local plan-retry <item-id>`. Manual retry retains committed and uncommitted
work on the original runner, with a fresh session reading the updated item.
An unfinished build or review, including a missing result report, receives up to
two continuations in its current stage before parking with an execution report.
A blocked build must identify the decision or prerequisite the owner must supply.
Quota limits and interrupted runners resume
