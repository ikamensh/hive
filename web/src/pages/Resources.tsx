...
import type { LocalRunnerInfo, MachineInfo, ResourceInfo, ResourcesPayload, RunnerInfo, StorageInfo } from "../types";
...
function StoragePanel() {
  const { data } = usePoll(() => api.storage(), []);
  ...
  const docTotal = Object.values(data.counts).reduce((sum, n) => sum + n, 0);
  ...
  return (
    <section className="storage-panel">
      <h2 className="col-title">storage</h2>
