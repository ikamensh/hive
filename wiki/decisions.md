...
## HIVE-MVP-009 · Completion note authorship
source_type: agent_proposed
impact: low · reversibility: high · status: accepted_for_iteration
expires_when: when the finish-iteration completion-note workflow is revisited or the operator asks for verifier-authored completion notes
trace: acceptance/finish-iteration.md#questions

The completion note (summary) is auto-generated by the orchestrator when calling `mark_goal_complete`, rather than being captured directly from the final verifier task's output. The orchestrator writes it based on the overall iteration summary and verification evidence.
