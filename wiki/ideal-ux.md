...
| G32 | Glance-formatter gaps: `hive plan` and `hive stories` still print raw JSON; plan verbs mix project-ids and item-ids with argparse walls on a wrong guess | all | low | done — `format_plan`/`format_stories` render readable rails; and the deeper bite found here: **every project-scoped verb 404'd on a project *name*** (only the 12-hex id worked) though `hive projects` shows names as the handle — `resolve_project_id` now maps name→id client-side (id-shaped tokens resolve for free), and an unknown name lists what exists instead of raw `{"detail":"Not Found"}` |
...
| G36 | Dead default backend hard-blocks projects that never asked for it: the build pipeline defaults every resolve/review task to `RESOLVE_BACKEND="codex"`, chosen statically not from the live fleet. On the 2026-07-22 gleaner run a plan item queued `on codex` and stuck at `blocked_resources` ("no online machine offers codex — log in a machine") though `gemini-cli @ hive-vm` was ready and gleaner's `required_capabilities` was empty | B,C | high | done — `_control/agent_choice.build_agent` substitutes a live backend when the requested one is unservable and grants don't pin it (priority codex>claude>cursor>gemini-cli); both pipelines route through it, `supervisor.available_backends` delegates to the shared helper. Proven live: retry rerouted the same item onto gemini-cli and it built |
| G37 | `hive iterate` (help: "start the next iteration") only resets the goal + wakes the planner async, but dumps the raw project JSON with no readable confirmation, and the project instantly reads `idle — no live plan; ask Hive to propose one` for the seconds the orchestrator is actually mid-propose — a "nothing is happening" reason that contradicts the queued wake | all | low | open |
| G38 | `Autonomy.pr` is a lie: the enum exists and gleaner is set to `pr` ("all code changes go through PRs for my review"), but `autonomy` is **never** consulted at landing and no code anywhere opens a GitHub PR — both pipelines always `POST /merges` (direct merge, no PR). Observed live 2026-07-22: gleaner iteration 2 direct-merged `feat: gleaner sessions` straight to `ikamensh/gleaner@main` (commits acc82f9+c580380), violating the stated posture. Archetype B promises "All landings are PRs in this posture." Design tension to resolve: PR mode conflicts with the "each sequential item branches from a default branch that already has prior items merged" invariant — a correct fix needs an `awaiting_merge` item state + a PR-merge poller to advance, not just swapping merge→PR (which would strand later items on un-merged main) | B,C | high | open |
